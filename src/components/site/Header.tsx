...
export function Header() {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 bg-background/85 backdrop-blur-xl border-b hairline">
        ...
        <div className="mx-auto max-w-[1400px] px-6 lg:px-10 flex items-center justify-between h-16 md:h-20 relative">
          ...
          <div className="flex items-center gap-5 text-foreground">
            <button onClick={() => setSearchOpen(true)} aria-label="Search" className="hover:text-accent transition-colors">
              <Search className="h-[18px] w-[18px]" />
            </button>
            <Link to={user ? "/seller" : "/sell"} aria-label="Seller" className="hidden md:inline-flex hover:text-accent transition-colors">
      ...
      {searchOpen && (
        <div className="fixed inset-0 z-50 bg-background/95 backdrop-blur-2xl animate-fade-up" style={{ animationDuration: ".25s" }}>
          <button onClick={() => setSearchOpen(false)} className="absolute top-6 right-6 hover:text-accent" aria-label="Close">
            <X className="h-6 w-6" />
          </button>
          <div className="mx-auto max-w-3xl px-6 pt-32">
            ...
            <form
              ...
              className="mt-6 flex gap-3 border-b-2 border-foreground pb-4"
            >
              <Search className="h-6 w-6 self-end mb-1 text-muted-foreground" />
              <input
                autoFocus
                value={q}
                onChange={(e) => setQ(e.target.value)}
                placeholder='Search "silk dress under 15000"…'
                className="flex-1 bg-transparent text-2xl md:text-3xl outline-none placeholder:text-muted-foreground/60"
              />
            </form>
            <div className="mt-8 grid grid-cols-2 md:grid-cols-3 gap-3 text-xs">
              {["silk dress under 10000", "leather bag", "gold earrings", "cashmere coat", "evening gown", "office accessories"].map((s) => (
                <button
                  ...
                  className="text-left p-3 border hairline hover:border-foreground transition-colors"
                >
                  {s}
                </button>
              ))}
